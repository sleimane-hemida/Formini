...
export default function Learning() {
    ...
    const handleTimeUpdate = () => {
        ...
        // ✅ FIX 1 : on vérifie que total est fini avant de calculer la progression
    ...
    const handleLoadedMetadata = () => {
        if (!videoRef.current) return;
        const total = videoRef.current.duration;
        // ✅ FIX 2 : on vérifie que la durée est valide avant de l'afficher
        if (isFinite(total) && total > 0) {
            setDuration(formatTime(total));
        }
    };

    // ✅ FIX 3 : handleProgressChange — le bug signalé dans la console
    // currentTime = NaN si duration n'est pas encore chargée → TypeError
    const handleProgressChange = (e) => {
        if (!videoRef.current) return;
        const videoDuration = videoRef.current.duration;
        // Garder uniquement si la durée est un nombre fini et non nul
    ...
    // ✅ FIX 4 : durée vidéo chargée → la barre de progression est interactive, sinon désactivée
    const isVideoReady = videoRef.current
        ? isFinite(videoRef.current.duration) && videoRef.current.duration > 0
        : false;
                    ...
                    <div className="px-8 pt-6">
                        ...
                        >
                            {contentType === "video" ? (
                                <>
                                    ...
                                    {/* Overlay lecture */}
                                    {!isPlaying && videoUrl && !loadingResources && (
                                        <div
                                    ...
                                    <div className={`absolute bottom-0 left-0 right-0 p-6 bg-gradient-to-t from-black/90 via-black/40 to-transparent transition-opacity duration-300 ${showControls ? 'opacity-100' : 'opacity-0'}`}>
                                        <div className="relative flex items-center gap-4 mb-4">
                                            ...
                                            <div className="flex-1 relative h-6 flex items-center">
                                                {/* ✅ FIX 3 appliqué ici : disabled si vidéo pas encore prête */}
                                                <input
                                                    type="range"
                                                    min="0"
                                                    max="100"
                                                    ...
                                                    className="video-slider w-full h-1 bg-white/20 rounded-full appearance-none cursor-pointer outline-none disabled:opacity-40 disabled:cursor-not-allowed"
                                        ...
                                        <div className="flex items-center justify-between">
                                            <div className="flex items-center gap-6">
                                                ...
                                                <button onClick={() => setIsPlaying(prev => !prev)} className="text-white hover:text-[#0C8CE9] transition-all transform hover:scale-110">
                                                ...
                                                <div className="flex items-center gap-3 group/vol">
                                                    <button onClick={() => setIsMuted(prev => !prev)} className="text-white/80 hover:text-white transition-colors">
                                ...
                                // ✅ Google Docs Viewer pour les PDFs Cloudinary
                        ...
                        <div className="flex items-center justify-between mb-8 bg-white p-4 rounded-xl shadow-sm border border-slate-100">
                            <div className="flex items-center gap-4">
                                <button
                                    disabled={!hasPrev}
                                    onClick={() => hasPrev && setActiveLecon(unlocked[activeIndex - 1])}
                                ...
                                <button
                                    disabled={!hasNext}
                                    onClick={() => hasNext && setActiveLecon(unlocked[activeIndex + 1])}
                            ...
                            <span className="text-[10px] font-bold text-slate-400 uppercase tracking-wider">
                                Leçon {activeIndex + 1} / {unlocked.length}
                            </span>
                        </div>
