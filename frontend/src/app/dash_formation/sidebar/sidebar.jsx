...
export default function Sidebar() {
	...
	return (
		<>
			...
			<aside className="hidden lg:block w-75 bg-[#14152D] rounded-md p-3 sticky top-6 border border-black/20 h-[calc(102vh-48px)]">
				<div className="flex flex-col h-full">
					<div className="flex items-center gap-3 px-2 py-3">
						<div className="w-14 h-14 rounded-full bg-gradient-to-br from-blue-600 to-blue-400 flex items-center justify-center text-white text-lg font-bold overflow-hidden border-2 border-white/10">
							{user?.photo ? (
						...
						<div>
							<div className="text-sm font-semibold text-white truncate max-w-[140px]">{user?.name || user?.username || 'Formateur'}</div>
							<div className="text-xs text-blue-300 font-medium">{user?.role || 'Instructeur'}</div>
						</div>
					</div>

					<div className="my-3 border-b border-white/5" />
					...
					<nav className="flex-1 py-2 space-y-2">
						{nav.map((item, idx) => {
							...
							return (
								<div
									key={item.id}
									className={`${idx !== nav.length - 1 ? 'border-b border-transparent' : ''} ${active ? 'bg-[#0C8CE9] rounded-md shadow-md shadow-blue-900/20' : ''} -mx-1`}>
										<button
										onClick={() => router.push(item.href)}
										className={`w-full flex items-center gap-3 px-4 py-4 transition-all duration-200 ${active ? 'text-white font-bold translate-x-1' : 'text-white/70 hover:text-white hover:bg-white/5 font-semibold'}`}>
										<span className={`inline-flex items-center justify-center w-9 h-9 rounded-md transition-colors ${active ? 'bg-white/20' : 'bg-white/10'}`}>
					...
					<div className="mt-auto px-2 pb-4">
						<button onClick={handleSignOut} className="w-full px-3 py-3 rounded-xl bg-red-500/10 text-red-400 hover:bg-red-500 hover:text-white transition-all duration-300 flex items-center justify-center font-bold text-sm border border-red-500/20 group">
							<FiLogOut className="inline-block mr-2 w-4 h-4 group-hover:-translate-x-1 transition-transform" /> Déconnexion
						</button>
					</div>
				</div>
			</aside>
						...
						>
							...
							<span className={`text-[10px] font-bold tracking-tight ${active ? 'text-[#0C8CE9]' : 'text-gray-500'}`}>
								{item.id === "dashboard" ? "Dashboard" : 
								 item.id === "profile" ? "Profil" :
								 item.id === "formations" ? "Formations" :
								 item.id === "facture" ? "Facture" : "Propulseur"}
							</span>
